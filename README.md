...
This is a hackathon build. The three components are structurally complete and were runnable end-to-end at submission time, but per an internal pass (`Remaining.md`), the App currently has failing TypeScript build/test checks, several UI sections that are still mock/placeholder-backed (network scanner, some collectors, settings toggles), and secrets that need rotating out of `App/.env` before any real deployment. See `Remaining.md` and `new_features.md` at the repo root for the full backlog.
